...
class ShardClient:
    ...
    def gauge_prompt_adherance(self, video_path: str, prompt: str):
        headers = {
            # "Authorization": f"Bearer {self.api_key}"
        }
        try:
            with open(video_path, 'rb') as f:
                ...
                response = requests.post(f"{self.api_url}/gauge_prompt_adherance", files=files, data=data, headers=headers)
                if response.status_code != 200:
                    raise APIError(f"Error: {response.json().get('error', 'Unknown error')}")
                task_id = response.json()['task_id']
                while True:
